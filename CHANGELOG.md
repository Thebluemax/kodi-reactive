...
- **Modal oculto tras el panel lateral**: El `<ion-modal>` se declaraba dentro de `album-detail`, que se proyecta dentro de `app-lateral-panel`. Un modal inline se queda donde se declara, así que heredaba el contexto de apilamiento y el `overflow` del cajón. Pasa a montarse en `album-list`, fuera del panel, y el detalle se limita a pedir la edición. De paso, `$lateral-panel-z-index` valía 40000 mientras los overlays de Ionic se montan a partir de 20000, así que el panel tapaba también toasts y alerts; baja por debajo de ese rango, y `lateral-slide` deja de repetir el número a mano (#200)

...
- **Modal oculto tras el panel lateral**: `$lateral-panel-z-index` valía 40000 y los overlays de Ionic se montan a partir de 20000, así que cualquier modal, alert o action sheet abierto desde dentro de un panel de detalle quedaba detrás de él. El panel baja por debajo de ese rango. `lateral-slide` repetía el 40000 a mano en vez de usar el token, y como lo usan `artist-list` y `genre-detail` habría reproducido el mismo fallo al llegar la edición de artista (#200)

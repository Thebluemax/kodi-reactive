...
- **Cobertura del reproductor y de los repositorios**: Los trece repositorios Kodi tienen ya pruebas; faltaban cinco, entre ellos el del reproductor y el de la cola. Se cubren además el facade de reproducción, los controles de reproducción y volumen, el servicio de tema y el de listas guardadas. De 42 archivos de prueba a 54, y de 339 pruebas a 414 (#236)
- **Capturas de pantalla en el paquete del add-on**: `addon.xml` declara cinco capturas —películas y álbumes en tema oscuro, mando a distancia, álbumes en tema claro y cola de reproducción— y el `Makefile` copia `resources/` al zip, que hasta ahora sólo llevaba `www/`, el icono y el propio `addon.xml`. Sin `<screenshot>`, Kodi no las muestra aunque viajen dentro. Las originales pesaban 40 MB en PNG a 4239×2246; reescaladas a 1920 y en JPEG suman 1,4 MB, así que el paquete pasa de 2,8 a 4,1 MB en lugar de multiplicarse por catorce (#226)
- **Exportar e importar la ficha de un medio**: El modal de edición gana «Exportar» y «Importar». Exportar descarga un JSON con los campos del medio y sus claves ya correctas; importar acepta ese JSON o un NFO de Kodi, y **rellena el formulario en vez de guardar**, así que lo importado pasa por el mismo diff que una edición a mano y hay ocasión de revisarlo antes. El formato se deduce del contenido y no de la extensión. Como se trabaja sobre el medio que ya está abierto, no hay nada que emparejar: ni rutas, ni títulos ambiguos, ni índice de biblioteca (#220)
...
- **Un solo cajón lateral**: `lateral-slide` era código de prototipo sin migrar —cinco `any`, `console.log` en cada cambio de entrada, métodos vacíos comentados— y hacía lo mismo que `lateral-panel`. Se retira, junto con el `SideBarService` y la directiva `lateral-slide-bar` que sólo él usaba, y sus tres consumidores pasan al panel. `lateral-panel` deja además de colgarse de `document.body` y se cuelga de `ion-app`, que es donde Ionic monta sus overlays: colgando del mismo contenedor, el z-index del panel compite con el de un modal o un alert en lugar de quedar en apilamientos separados (#231)
- **Encuadre del cartel en película y serie**: `app-media-tile` fijaba `background-size: cover` en la plantilla y no ponía posición. La tarjeta es cuadrada, así que un cartel vertical se ampliaba hasta llenarla y se recortaba **desde la esquina superior izquierda**, perdiendo justo el centro. El tamaño pasa a ser una entrada del componente: película y serie muestran el cartel entero a lo alto y centrado, y álbum, actor y género de vídeo se quedan como estaban, con sus carátulas cuadradas y sin posición fijada (#224)
...
- **Los WebSocket no reconectaban**: Si Kodi se reiniciaba, la red parpadeaba o el equipo suspendía, el reproductor dejaba de actualizarse hasta recargar la página, y sin nada en la interfaz que lo dijera: el `onclose` sólo marcaba el estado. Había además dos conexiones al mismo Kodi, una para el reproductor y otra para la biblioteca, que sólo vivía mientras Ajustes estaba a la vista. Ahora hay una sola, compartida, que reconecta espaciando los reintentos hasta un tope —de un segundo a treinta— y la barra superior dice cuándo no hay conexión y cuándo está reintentando. Los eventos de biblioteca escuchan desde el arranque, no sólo en Ajustes (#235)
- **Errores de Kodi tratados como éxito**: Kodi responde a los rechazos con HTTP 200 y el fallo dentro del sobre JSON-RPC, así que no llegan como error de red. Cinco repositorios no lo miraban en ninguna petición: `player` e `input` daban por buena una orden rechazada —el mando parecía no hacer nada, sin aviso— y `genre`, `actor` y `video-genre` reventaban al leer `result.algo` sobre un `undefined`, con un «Cannot read properties of undefined» que no dice nada. Ahora toda petición pasa por un desenvolvedor compartido que lanza con el mensaje y el código de Kodi. De paso desaparecen los dos `(response as any)` que esquivaban el tipado (#233)
- **Textos en inglés en la interfaz**: El detalle de artista tenía ocho cadenas sin traducir —«Play Artist», «Add to Playlist», «Play Album», «Play Track»— y las pestañas de navegación de música y vídeo estaban enteras en inglés: «Movies», «TV Shows», «Actors», «Genres», «Albums», «Artists». Toda la interfaz queda en castellano (#232)
- **Un fallo de conexión se presentaba como biblioteca vacía**: Las listas trataban el error como una respuesta sin datos —`catchError` devolvía `EMPTY`— así que con Kodi apagado se leía «No hay películas en la biblioteca», que es mentira y parece un fallo del add-on. `app-empty-state` gana un modo de error, con su propio icono, el motivo del fallo y un botón de reintentar, y el fallo manda sobre el filtro activo porque sin datos no se puede afirmar que la búsqueda no encontrara nada. Las siete listas lo usan. Además, los errores de las acciones del usuario —reproducir, añadir a la cola, abrir un detalle— pasan de `console.error` a un aviso visible: eran 66 llamadas a `console.*` en producción y ni un mensaje, teniendo `NotificationService` desde #198 (#230)
- **El scroll infinito pedía una página de más y duplicaba la lista**: El guard comparaba `start`, que apunta al principio de la página **ya pedida** y no a la siguiente. Con 80 elementos y páginas de 40, tras la segunda carga `start` valía 80 y el guard seguía dando `true`, así que se pedía el rango 80–120 sobre 80 elementos; Kodi responde a un rango fuera de límites con la lista completa, y el handler la appendea. Ahora se corta por la cantidad realmente cargada, y una respuesta vacía marca el final para que un total optimista no deje el scroll pidiendo indefinidamente. El appendeo descarta además por identificador lo que ya estuviera. Afectaba igual a álbumes y series (#228)

...
- **Tests de configuración de Kodi**: `KodiConfigService` no tenía cobertura. El spec nuevo inyecta el servicio real, sin sustituirlo, para fijar que el entorno de tests es el mismo en local y en CI (#195)
...
- **La configuración `ci` compilaba y testeaba como producción**: No definía `fileReplacements`, así que caía en el archivo base `environment.ts`, que declaraba `production: true` con valores de desarrollo. `ng test --configuration=ci` —el job de CI y el gate de `release.yml`— hacía que `KodiConfigService` tomara la rama de `window.location`, que bajo Karma resuelve al servidor de Karma (`localhost:9876`) en vez de al proxy. No rompía nada porque los specs que lo tocaban lo sustituían, pero el primer spec que inyectara el servicio real habría obtenido URLs distintas en local y en CI. Ahora cada configuración de `angular.json` nombra su entorno de forma explícita y ninguna depende del archivo base (#195)
